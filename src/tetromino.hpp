...
namespace tetris
{
  ...
  {
  public:
    Tetromino(const LogicType &type, const int x = 0, const int y = 0) : type_(type), discretePosition_(x, y)
    {
      setShape_(type);
    }
    Tetromino(const LogicType &type, const Coord &coord) : type_(type), discretePosition_(coord)
    {
      setShape_(type);
    }
    ...
    {
      // (X, Y) --(rotation clock)--> ((L - 1) - Y, X), with L the length of the square map.
      // data that was at rank X + Y * L is now at rank ((L - 1) - Y) + X * L
    ...
    void updatePosition(const Coord &coord)
    {
      discretePosition_ = coord;
    }
    void addPosition(const Coord &coord)
    {
      discretePosition_ += coord;
    }
  ...
  private:
    ...
    void setShape_(const LogicType &type)
    {
      ...
      case LogicType::Square:
        ...
        // set(LogicType::Square, static_cast<std::vector<Coord>>( { {1,1}, {1,2}, {2,1}, {2,2} } ));
  ...
  {
    static constexpr int shapeNb = 7;
    ...
    const int randomShapeRk = randomGen.generateRandomInteger(shapeNb - 1); // range [0, shapeNb - 1]
