...
template <int xDim_, int yDim_> class TetrisGame {
public:
  ...
  TetrisGame()
      : gameDisplay_(*this), // open the window
        window_(gameDisplay_.getWindow()),
        // init game status
        gameOver_(false), score_(0), // init score
        ...
        coord0_(x0, y0), tetromino_(randomShape(), coord0_) {}
...
private:
  ...
  enum CollisionType { Wall, Bottom, Tetro, GameOver, OutOfBound, None };

  void closeGame_() { gameDisplay_.closeWindow(); }
  ...
  void tick_() { moveWithCheck_(Coord::Down); }

  void setGameOver_() { gameOver_ = true; }
  ...
  void drop_() {
    ...
    } while (collision != CollisionType::Bottom &&
             collision != CollisionType::Tetro);

    if (collision == CollisionType::GameOver) {
      setGameOver_();
    }

    tetromino_.addPosition(
        Coord::Up); // move one step up to cancel the last forbidden move
  ...
  void fixTetromino_() {
    ...
    for (localCoord.x = 0; localCoord.x < 4; localCoord.x++) {
      for (localCoord.y = 0; localCoord.y < 4; localCoord.y++) {
        const auto mapType =
            logicMap_.get(localCoord + globalCoord); // logical type of the map
        if (mapType ==
            LogicType::Void) // skip if out of bound (if a coordinate of
                             // tetromino's map is one block away from walls)
        {
  ...
  void moveWithCheck_(const Coord &coord) {
    ...
    case CollisionType::Tetro:
      ...
      if (coord.y > 0) {              // to avoid fix on lateral move
        fixAndResetTetromino_(randomShape());
      }
      break;
  ...
  CollisionType checkCollision_() {
    // the tetromino in a candidate new position can collide with three types of
    // block (aside from void)
    bool gameOver = false;
    bool wall = false;
    bool tetro = false;
    bool bottom = false;
  ...
  void localCheck_(int x, int y, const Coord &tPos, bool &gameOver, bool &wall,
                   bool &tetro, bool &bottom) {
    const auto mapType =
        logicMap_.get(x + tPos.x, y + tPos.y); // logical type of the map
    if (mapType !=
        LogicType::OutOfBound) // skip if out of bound (if a coordinate of
                               // tetromino's map is one block away from walls)
    {
      ...
      if (tType != LogicType::Void &&
          mapType != LogicType::Void) // If one is void, nothing to test
      {
        switch (mapType) {
        case LogicType::Wall:
          wall = true;
