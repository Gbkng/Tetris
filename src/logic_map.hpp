...
/**
 * The logicMap essentially consists in an 2D array with each case representing
 * the status of the corresponding block.
 *
 * The tricky part here is that the origin is not in the top-left corner, in
 * order to have a convenient coordinate system for tetrominos.
 *
 *     -1  0   1   2   3   4
 *    |---|---|---|---|---|---|
 * -1 | 0 | 1 | 2 | 3 | 4 | 5 |
 ...
 * The top domain of equation (y = -1 and x!=4 and x!=-1) is filled with
 * GameOver type (G).
 *
 *     -1  0   1   2   3   4
 *    |---|---|---|---|---|---|
...
class LogicMap
    : public Map<LogicMap<xDim, yDim>, LogicType, xDim + 2, yDim + 2> {
public:
  ...
  int coordToRank(const int x, const int y) const {
    ...
    if constexpr (verbose) {
      ...
      if (x == -1 || x == xDimTot - 1 || y == -1 || y == yDimTot - 1) {
        std::cout << "Careful, requesting an out of tetromino's domain area."
                  << std::endl;
